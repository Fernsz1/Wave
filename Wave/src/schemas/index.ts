 ...
 *
 * Zod schemas for the canonical wire models. Used to VALIDATE objects decoded
 * from the codec at the sync boundary — a frame off MQTT/LoRa is untrusted
 * input. The UI's internal types stay in ./types.ts.
 */
import { z } from 'zod';
...
export const SummativeConfigSchema = z.object({
  total: z.number().int(),
  passMark: z.number().int(),
  itemsPerTopic: z.number().int().optional(),
...
export const QuizAttemptRequestSchema = z.object({
  ...
  mode: ModeSchema,
  quizzesPerTopic: z.number().int().optional(),
  summativeConfig: SummativeConfigSchema.optional(),
  seed: z.number().int(),
  focusTopicIds: z.array(z.string()).optional(),
});
...
export const SCHEMA_BY_TYPE = {
  ...
  QuizAttemptRequest: QuizAttemptRequestSchema,
